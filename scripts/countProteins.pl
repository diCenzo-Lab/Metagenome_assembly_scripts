...
$parent = "$pwd/phylogeny/markerScannerSingle"; # directory with the MarkerScanner
...
sub read_files {
	...
	close($fh);
	if($count / $total >= 0.90) {
		system("mv $pwd/phylogeny/markerScannerSingle/$filename $pwd/phylogeny/markerScannerCounted/");
	}
	$count = "0";
}
